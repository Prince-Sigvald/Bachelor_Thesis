...
Description: Combines all important files to let Lio dock onto a laundry cart 
which equppied with an aruco marker
Docking process happens iterative, currently with three iterations

Note: This file serves only as an example usage and should present how the class, scripts and functions
...
Description: The class is needed to compute all important linear and angular navigation values for Lio
to dock to a laundry cart with an aruco marker
The computed values are stored as members of the class

Note: The class members from row 28 should not be changed, especially all the distance values were optimised
...
Description: The file contains all functions to execute the driving commands which were computed 
by the previously described class

Note: It is recommended to first have a look at Class_Compute_Navigation_Docking to understand 
...
Description: The script lets Lio drive two meters forward and lets the parallel script check_pulling_force run
to check if the movement interruption operates well

Note: The shared variable guarantees, that the releasing routine can be finsihed in the parallel script


---------------------------------------------------------------------------------
File: check_pulling_force

Description: This script is a parallel script and enables the 12V buck converter on the electric interface
...
Note: The shared variable in the script is only used for testing reasons
